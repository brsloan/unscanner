...
| `src/unscanner/webapp.py` + `web/` | local web UI (FastAPI + one plain HTML/JS page, no build step); mounts the MCP server at `/mcp`. `serve` shows it in a pywebview window when the optional `window` extra is installed (the server then runs on a thread; closing the window stops it), else in the browser (see `desktop.py`) |
