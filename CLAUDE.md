...
| `website/` + `.github/workflows/pages.yml` | the project's public page on GitHub Pages: one static `index.html` (no build step, no framework) with the two app screenshots and a copy of `favicon.svg`; pushed to Pages whenever `website/` changes on main. Its claims describe features, so change the page when a feature it names changes |
